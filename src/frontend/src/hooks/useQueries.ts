...
export function useSetInventoryStock() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const success = await actor.setStock(plant, productName, quantity, user);
      if (!success) {
        throw new Error(
          `Update failed: plant "${plant}" or product "${productName}" was not found in the backend inventory. Please ensure the inventory has been initialized with the correct plant and product entries.`,
        );
      }
      return success;
    },
...
export function useSetBardanaStock() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!success)
        throw new Error(
          "Failed to update bardana stock — product not found or invalid quantity",
        );
      return success;
    },
    onSuccess: () => {
...
export function useSetRawMaterialStock() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not ready");
      const success = await actor.setRawMaterialStock(
        plant,
        productName,
        quantity,
      );
      if (!success)
        throw new Error(
          "Failed to update raw material stock — product not found or invalid quantity",
        );
      return success;
    },
    onSuccess: () => {
