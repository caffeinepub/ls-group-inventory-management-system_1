...
import { Loader2, FlaskConical, Pencil, Check, X } from 'lucide-react';
import { useGetRawMaterials, useSetRawMaterialStock } from '@/hooks/useQueries';
...
const RAW_MATERIALS_ORDER = [
  'Vegetable Oil', 'Diesel', 'Firewood', 'Glucose',
  'Colour', 'Thread', 'Emery', 'Salt', 'Cement',
];

function PlantRawMaterials({ plant, products }: { plant: string; products: ProductView[] }) {
  const [editingProduct, setEditingProduct] = useState<string | null>(null);
  const [editValue, setEditValue] = useState<string>('');
  const setStock = useSetRawMaterialStock();

  const sorted = sortProducts(products, RAW_MATERIALS_ORDER);
...
export default function RawMaterialsTab() {
  const { data: rawData, isLoading } = useGetRawMaterials();

  const getPlantProducts = (plant: string): ProductView[] => {
    if (!rawData) return RAW_MATERIALS_ORDER.map((name) => ({ name, quantity: 0 }));
    const entry = rawData.find(([key]) => key === plant);
    if (!entry) return RAW_MATERIALS_ORDER.map((name) => ({ name, quantity: 0 }));
    const productMap = new Map(entry[1].products.map((p) => [p.name, p]));
    return RAW_MATERIALS_ORDER.map((name) => productMap.get(name) ?? { name, quantity: 0 });
  };
          ...
          <Tabs defaultValue="ls-pulses" className="w-full">
            ...
            {PLANTS.map((plant) => (
              <TabsContent
                ...
                className="mt-0 pt-3"
              >
                <PlantRawMaterials plant={plant} products={getPlantProducts(plant)} />
              </TabsContent>
            ))}
          </Tabs>
        )}
